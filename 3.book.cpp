...
void Gll::insertb() // to insert book node
{      
         if(head==NULL) // if no node created
           { 
            t1=create(); // creat node t1 & it is head node
                head=t1;
           }
        else
           {
...
cout<<"\n how many chapters you want to insert";
                	...
                	{
               		 t1=create();
                		if(head->flag==0) // if flag 0 no chapter under head node
                		{  
head->down=t1;   head->flag=1; //then below that create chapter node & get flag of head=1    
}
               		 else
               		 {  
 temp=head;
...
temp=temp->down;
                		while(temp!=NULL)
                 		{ 
                      		if(!strcmp(ch,temp->name)) // check for chapter name gives in ch
                      		{   
                                		cout<<"\n how many sections you want to enter";
                              		 cin>>n; // enter no of section
                              		 for(i=0;i<n;i++)
                             			{
                                          			 t1=create();
                                              		 if(temp->flag==0)
